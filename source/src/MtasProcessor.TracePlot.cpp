...
//	DeclareHistogram1D(MTAS_POSITION_ENERGY+307, EnergyBins, "Scalar Rate (s) Beta Gated Central");
  ...
  DeclareHistogram2D(MTAS_POSITION_ENERGY+360,SB,SE,"Si Raw Energy/10 vs. Time(min)");
 	DeclareHistogram2D(MTAS_POSITION_ENERGY+361,SB,SA,"Si Energy from Traces/10 vs. Time");
  ...
  DeclareHistogram2D(MTAS_POSITION_ENERGY+368, S9,SB, "Region 3 TraceE/10 v MTAS/10 ");
	...
	DeclareHistogram2D(MTAS_POSITION_ENERGY+393,SD,SC,"Si E/10 vs. Post - Pre Baseline");
	...
	DeclareHistogram1D(MTAS_POSITION_ENERGY+519, S4, "Silicon Position, left chan - top detector");
...
bool MtasProcessor::Process(RawEvent &event)
{
	...
	map<string, struct KSIData> siliMap;
...
//Matt** Pairing PMT's 
  ...
  double actualTime = -1.0;   
	...
	} else if (siliSummary->GetMult() > 0)//I have at leat one element in mtasList
	{
...
//		SiSat = (*siliListIt)
		...
		siliMap.insert(make_pair(subtype,KSIData((*siliListIt))));
	}
	...
	/**if(siliMap.size()>0)
	{
		...
		if(siliMap.size()>0)
	  {
            const ChanEvent *siliEv = siliSummary->GetMaxEvent();
            maxSiliconSignal = (siliEv->GetCalEnergy());//Jan 03 2011 Ola K
            siliSaturated = siliEv->IsSaturated();
            siliAvgBaselinePost = siliEv->GetAveBaselinePost();
            tAvgBaseline = siliEv->GetAveBaseline();
            tRegion = siliEv->GetRegion();

            if(maxSiliconSignal > 0.0 || siliSaturated) 
            {
            		// TRADITIONAL set of beta signal BCR
               // isBetaSignal = true; 
                //int phase = siliEv->GetPhase();
                int satLen = siliEv->GetSatLen(); // Returns number of sat. channels. NTB 1/22/18
                betaTime = (siliEv->GetTime()) * pixie::clockInSeconds;
              if (siliSaturated)
              {
                  maxSiliconSignal+= maxSiliconSignal*(satLen*satLen)/750000.;//gaussian approx. 8*sigma^2 = 8*350^2 for KSI NTB
                  plot(MTAS_POSITION_ENERGY+393,maxSiliconSignal/10., (siliAvgBaselinePost-tAvgBaseline));
              }
              plot(MTAS_POSITION_ENERGY+390, satLen);
                  //else if (siliSaturated) maxSiliconSignal+=satLen+10; //moves energy away from the end of spectra and makes it seperable. NTB
    ...
    plot(MTAS_POSITION_ENERGY+392,siliAvgBaselinePost-tAvgBaseline);
    plot(MTAS_POSITION_ENERGY+391,maxSiliconSignal/10);
  ...
  for(map<string, struct MtasData>::const_iterator siliMapIt = siliMap.begin();
            siliMapIt != siliMap.end(); siliMapIt++)
	{
	
	}*/ //CHECK THIS NTB
	
	int siliMult = siliMap.size(); 
	plot(MTAS_POSITION_ENERGY+500, siliMult);
	
	double siliSumEnergy = 0.0;
  ...
  for(map<string, struct KSIData>::const_iterator siliMapIt = siliMap.begin();
            siliMapIt != siliMap.end(); siliMapIt++)
	{
		...
		if (subtype == "SK1" && (*siliMapIt).second.calEnergy > 0. )
		{
		    sili1Energy = (*siliMapIt).second.calEnergy;
		    trace1 = (*siliMapIt).second.trace;
		} else if (subtype == "SK2" && (*siliMapIt).second.calEnergy > 0. )
		{
		    sili2Energy = (*siliMapIt).second.calEnergy;
        trace2 = (*siliMapIt).second.trace;
		}
    plot(MTAS_POSITION_ENERGY+510, (*siliMapIt).second.location-76);//hardcoded numbering from map file! NTB
	}
  ...
  if (siliMult > 1) 
  {
      ...
      if( siliSumEnergy > 0 ) //hardcoded cutoff? NTB
      {
	      plot(MTAS_POSITION_ENERGY+507, 1000.0 * ( sili1Energy - sili2Energy ) / siliSumEnergy + 2000.0 );
	      plot(MTAS_POSITION_ENERGY+508, siliSumEnergy, siliPDiff);
	      //plot(MTAS_POSITION_ENERGY+509, siliSumEnergy, siliPDiff );
	      if (siliPDiff > 2000 && siliPDiff < 2500) plot(MTAS_POSITION_ENERGY+360, siliSumEnergy/10 , (betaTime-firstBetaTime)/60.);
	      if (tRegion ==1) plot(MTAS_POSITION_ENERGY+511, siliSumEnergy);
 	      if (tRegion ==2) plot(MTAS_POSITION_ENERGY+512, siliSumEnergy);
 	      if (tRegion ==3) plot(MTAS_POSITION_ENERGY+513, siliSumEnergy);
 	      if (tRegion ==4) plot(MTAS_POSITION_ENERGY+514, siliSumEnergy);
      }
      maxSiliconSignal = siliSumEnergy; //using avg as energy! NTB
  } else if (siliMult >0)
  {
      plot(MTAS_POSITION_ENERGY+503, sili1Energy);
      plot(MTAS_POSITION_ENERGY+504, sili2Energy);
      plot(MTAS_POSITION_ENERGY+361, maxSiliconSignal/10 , (betaTime-firstBetaTime)/60.);
      if (tRegion ==1) plot(MTAS_POSITION_ENERGY+515, maxSiliconSignal);
      if (tRegion ==2) plot(MTAS_POSITION_ENERGY+516, maxSiliconSignal);
      if (tRegion ==3) plot(MTAS_POSITION_ENERGY+517, maxSiliconSignal);
      if (tRegion ==4) plot(MTAS_POSITION_ENERGY+518, maxSiliconSignal);
...
//spectras 
    ...
    if(isBetaSignal)
    {
    	...
    	if(  totalMtasEnergy.at(0) ==0 && siliSumEnergy > 0) plot(MTAS_POSITION_ENERGY+509, siliSumEnergy, siliPDiff ); // totalMtasEnergy.at(0) == 0 NTB
    	if(  totalMtasEnergy.at(0) > 0 && siliSumEnergy > 0) plot(MTAS_POSITION_ENERGY+510, siliSumEnergy, siliPDiff ); // totalMtasEnergy.at(0) > 0 NTB siliSumEnergy >0 redundant?
    }
		...
		if(isBetaSignal) 
		{
            ...
            if (totalMtasEnergy.at(0) < 1 && siliSumEnergy < 2000)
            {
                if (siliSumEnergy<500) 
                {
                  ...
                  {
                    plot(MTAS_POSITION_ENERGY+520,trace1[i],rowNoise);
                    plot(MTAS_POSITION_ENERGY+520,trace2[i],rowNoise+1);
                  }
                  rowNoise+=2;
                //cout << trace1[10] << " noise" << endl;                  
                } else {
                  for(unsigned int i = 0; i < trace1.size(); i++)
                  {
                    plot(MTAS_POSITION_ENERGY+ 521,trace1[i],rowSignal);
                    plot(MTAS_POSITION_ENERGY+ 521,trace2[i],rowSignal+1);
                  }
                  rowSignal+=2;
               // cout << trace1[10] << " Signal" << endl;                                    
                }
            }
...
//          const bool noCenter = totalMtasEnergy.at(1) < 1.0;
		...
		for(map<string, struct KSIData>::const_iterator siliMapIt = siliMap.begin(); siliMapIt != siliMap.end(); siliMapIt++)
		{
			siliconNumber = 0;
            ...
            plot(MTAS_POSITION_ENERGY+519, siliconNumber);
		}
