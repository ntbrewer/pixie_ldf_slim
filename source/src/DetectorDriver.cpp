...
//  static int i;
//   if(i++%100==1)
//    vecProcess.push_back(new ThermometerProcessor());
...
{   
	...
	"dssd_front", "dssd_back", "dssd_front", "position", "timeclass",
	"ge", "si", "scint", "mcp","mtc", "mtas","ssd","generic", "vandle", "pulser", "sili", "logi","refmod","therm"}; 
...
{   
    ...
    */
    ...
    if ( !chan->GetTraceRef().empty() ) {
        ...
        double avgBLPost = traceSub.GetAvgBaselinePost();
        double avgBL = traceSub.GetAvgBaseline();
        //double stdDevBL = traceSub.GetStdDevBaseline();
        int traceMax = traceSub.GetTraceMax();
        int tRegion =  traceSub.GetRegion();
        //double phase =  traceSub.GetPhase();
        int satLen = traceSub.GetSatLength(); // NTB 1/22/18
        
        chan->SetSatLen(satLen);//NTB use GetSatLen() to retrieve.
        chan->SetAveBaseline(avgBL);
        chan->SetAveBaselinePost(avgBLPost);
        chan->SetMaxValue(traceMax);
        chan->SetRegion(tRegion);
        traceEnergy = traceSub.GetEnergy();
        chan->SetTrcQDC(traceEnergy);
        //chan->SetPhase(-phase/4);
        //chan->SetPhase(-200);
        //cout << "phase" << subtype << " "<< id <<endl;
        /**/
        energy = chan->GetEnergy() + randoms.Get(); //NTB !!!SKIP TRACE ANALYSES!!TBD
        energy /= ChanEvent::pixieEnergyContraction;
        plot(dammIds::trace::DD_ENERGY_V_TMAX, energy/10, traceMax-avgBL);
        energy = traceMax - avgBL; //NTB 
        energy *= 8.; //scale factor for ( ADC MAX )/ ( trace height max ) -> 2**15/2**12 == 8
        //energy /= ChanEvent::pixieEnergyContraction;
    } else {
...
int DetectorDriver::PlotRaw(const ChanEvent *chan) const
{
    ...
    int isSaturated = chan->IsSaturated();
    plot(dammIds::misc::offsets::D_RAW_ENERGY + id, energy);
    if (isSaturated)
    {
        plot(dammIds::misc::D_SAT_ID,id);
    } else {
        plot(dammIds::misc::D_NOTSAT_ID,id);
    }
    ...
    for(unsigned int a = 0; a < numCal; a++) {
        ...
        if (raw >= thresh[a] && raw < thresh[a+1]) {
            ...
            for(unsigned int b = 0; b < polyOrder+1; b++) {
		      ...
		      //  cout << val[a*(polyOrder+1) + b] << ' ' << calVal << ' '<< b << ' '<< raw<< endl;
            }
	    break;
        }
    }
