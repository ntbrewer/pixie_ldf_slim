...
/******************************************************************************
*																																							*
*			CLEANED CODE FROM ABOVE. PREVIOUS LOCATION TAGGED WITH CLEAN_#					*
*																																							*
******************************************************************************/
...
//CLEAN_9
/*  // Need to process the whole event, not just the max event  - KSI 2018-02-13 NTB / DVM
		if(siliMap.size()>0)
	  {
            const ChanEvent *siliEv = siliSummary->GetMaxEvent();
            maxSiliconSignal = (siliEv->GetCalEnergy());//Jan 03 2011 Ola K
...
/*
			if (tRegion ==1) 
			{
				plot(MTAS_POSITION_ENERGY+511, siliSumEnergy);
			}
			if (tRegion ==2)
			{
				plot(MTAS_POSITION_ENERGY+512, siliSumEnergy);
			}
			if (tRegion ==3)
			{
				plot(MTAS_POSITION_ENERGY+513, siliSumEnergy);
			}
			if (tRegion ==4)
			{
				plot(MTAS_POSITION_ENERGY+514, siliSumEnergy);
			}
...
//CLEAN_18
	else if (siliMult >0)
  {
		...
		if (tRegion ==1)
		{
			plot(MTAS_POSITION_ENERGY+515, maxSiliconSignal);
		}
		if (tRegion ==2)
		{
			plot(MTAS_POSITION_ENERGY+516, maxSiliconSignal);
		}
		if (tRegion ==3)
		{
			plot(MTAS_POSITION_ENERGY+517, maxSiliconSignal);
		}
		if (tRegion ==4)
		{
			plot(MTAS_POSITION_ENERGY+518, maxSiliconSignal);
		}
  }
*/
//CLEAN_19 Not necessary for KSI

	//silicon spectras
//	plot(MTAS_POSITION_ENERGY+500, siliMap.size()); //duplicated
	//int siliconNumber;
